...
/// 填字/猜词游戏设置界面
class PuzzleSettingsScreen extends ConsumerStatefulWidget {
  final int childId;

  const PuzzleSettingsScreen({super.key, required this.childId});

  @override
  ConsumerState<PuzzleSettingsScreen> createState() => _PuzzleSettingsScreenState();
}

class _PuzzleSettingsScreenState extends ConsumerState<PuzzleSettingsScreen> {
  late IdiomGameSettingsDao _settingsDao;
  bool _isLoading = true;

  // 临时配置状态
  int _countdownSeconds = 60; // Default for Puzzle
  ...
  Future<void> _saveSettings() async {
    await _settingsDao.upsertSettings(
      ...
      // Keep other fields (grade, hints, match mode) as they were or default if new
      // We only care about countdown and rare idioms for this screen.
      // upsertSettings handles defaults for new records.
    );
